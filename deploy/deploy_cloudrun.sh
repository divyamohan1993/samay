...
CORS="${CORS:-https://samay.dmj.one,https://dmj.one}"
...
echo "  Add a Cloudflare CNAME:  samay  ->  ghs.googlehosted.com  (proxied)"
echo "  then map the custom domain:  gcloud run domain-mappings create --service $SERVICE --domain samay.dmj.one --region $REGION"
